...
class RPPGProcessor:
    ...
    def process_batch(self):
        ...
        # 轻微高斯平滑（消除超像素接缝锯齿）
        norm_energy = cv2.GaussianBlur(norm_energy, (3, 3), 0)
        ...
        boundaries = skimage.segmentation.mark_boundaries(
            np.zeros((ROI_H, ROI_W, 3)), segments, mode='outer'
        )
        bound_mask = boundaries[:,:,0] > 0
        self.heatmap_result[bound_mask] = self.heatmap_result[bound_mask] * 0.4
    ...
    def process_frame(self, frame_bgr: np.ndarray) -> np.ndarray:
        ...
        if self.state == 0:
            ...
            cv2.putText(
                output, f"RECORDING... {15 - (self.frame_count/self.fps):.1f}s", 
                (ROI_X, ROI_Y - 30), 
            ...
            if self.frame_count >= self.total_frames:
                ...
                # 融合超像素热像图与原始 ROI 坐标
                bg[ROI_Y:ROI_Y+ROI_H, ROI_X:ROI_X+ROI_W] = self.heatmap_result
